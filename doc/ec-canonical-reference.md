# Epimethean Challenge — Canonical Reference

This document is the single, reconciled reference for Epimethean Challenge (EC).
It consolidates the taxonomy, unit list, noun extraction, mass/volume tables,
transport rules, combat phases, turn-processing sequence, and order syntax into
...
Birth Rate increases are based on the amount of habitable land unpopulated, and inversely on the SOL.
...
| Code    | Name        | Description                                                                                      |
|---------|-------------|--------------------------------------------------------------------------------------------------|
| SDRV-TL | Space Drive | Maintains orbit and manuvers in combat; cannot be used for interplanetary or interstellar travel |
| HDRV-TL | Hyper Drive | Propels Ships through hyperspace; jump range = 1 light-year × TL                                 |
| TRNS-TL | Transports  | Transfer Population and materials between Ships/Colonies at the same Planet (see §7)             |
...
- **Storage vs. operational volume (open):** Units in storage count as only ½ their
  mass for structural housing purposes, implying storage and operational volume are
  not equivalent. The VU column above does not yet distinguish the two states; this
  requires further research (see §12).
...
**Normal operations** — transfer rate is a *throughput*, not a single-lift capacity:

> **TL² × 200 MU per turn**

A TRNS-2 moves 800 MU per turn by making many trips within the turn (batches of up
to its physical volume, ~12 VU each), not by carrying 800 MU at once. The turn is
long enough to accommodate the repeated trips.

**Combat operations** — only a single trip is possible per round:

> **3 × TL MU per combat round**

One load, one run — consistent with the compressed timescale of a combat round.
...
- **TRNS-10 moving 20,000 USK:** throughput `10² × 200 = 20,000 MU/turn`;
  20,000 VU ÷ 60 VU batch ≈ **333 trips**. The high TL justifies the tempo (TL²
  throughput scaling: a TRNS-10 moves 25× a TRNS-2 with only 5× the volume). In
  combat: `3 × 10 = 30 MU` per round.
...
state left by all earlier stages. This ordering is what makes production available
before it is spent, combat resolve before survivors act, and population change last.
...
| 3 | Combat | All combat is resolved (see §10.5 for the per-round phase sequence). |
...
> **Notes:**
> - Production (stages 1–2) precedes everything, so newly produced resources and
>   units are available to later stages the same Turn.
> - Combat (stage 3) resolves before set-up, transfer, and assembly, so only
...
Combat is resolved in stage 3 of the Turn Processing Sequence (§10.4). The following
...
- **Intercept resolution (step 3)** is implicit in the source text; it may be
  modelled as a sub-step of weapons fire or kept separate depending on damage
  sequencing needs.
